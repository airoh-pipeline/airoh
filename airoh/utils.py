...
#
# `download_data` and `ensure_submodule` moved to `airoh.acquisition`. Importing
# them from `airoh.utils` (or invoking them as `utils.download-data` /
# `utils.ensure-submodule`) still works but is deprecated — prefer
# `from airoh.acquisition import ...`. Remove this shim once dependent projects
# have migrated.
from airoh.acquisition import download_data, ensure_submodule  # noqa: F401
